...
for /f "tokens=* delims= " %%x in (%a%.nx) do (
	...
	for %%a in (%%x) do (
		...
		if %%a == reverse set printString=!printString:reverse=call fclib_string_reverse.bat!
		if %%a == factorial set printString=!printString:factorial=call fclib_math_fact.bat!
		if %%a == length set printString=!printString:length=call fclib_string_length.bat!
		if %%a == toUpperCase set printString=!printString:toUpperCase=call fclib_string_upper.bat!
		if %%a == toLowerCase set printString=!printString:toLowerCase=call fclib_string_lower.bat!
		...
		if %%a == arrMax set printString=!printString:arrMax=call fclib_array_max.bat!
		if %%a == arrMin set printString=!printString:arrMin=call fclib_array_min.bat!
		if %%a == arrSum set printString=!printString:arrSum=call fclib_array_sum.bat!
		if %%a == arrLength set printString=!printString:arrLength=call fclib_array_len.bat!
		...
		if %%a == while (
			...
			echo if !fccondition! goto EndLoop!wloopnum!>>!outtar!
			set deniedToken=true
		)
		if %%a == endloop (
			if "!procadd!" == "true" (
