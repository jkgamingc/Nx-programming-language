...
if "%1" == "" (
	...
	echo  --version  :Have to be the first parameter, shows the current version of the Nx compiler.
...
for /f "tokens=* delims= " %%x in (%a%.nx) do (
	...
	for %%a in (%%x) do (
		...
		if %%a == null set printString=!printString:null=!
		if %%a == // set printString=!printString://=::!
	)
